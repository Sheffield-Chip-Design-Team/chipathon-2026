...
3rd order feed-forward sigma-delta modulator converting int16 combined samples back to 1-bit I+Q bitstreams at 32 MS/s, suitable for the SX1302 Radio A/B inputs. Two instances: re-mod A (always active) and re-mod B (NT=2 mode only).
...
OSR = 32 (32 MS/s / 1 MS/s). In-band SQNR > 100 dB for a 3rd order modulator at OSR=32 — int16 input is conservative.
...
**Clock domain.** Input is at 1 MS/s (in_valid strobe); modulator runs at 32 MHz. On `in_valid`, latch the input into a register and run the 3rd order loop at 32 MHz for the next 32 cycles.

**Mode 1.** Set `en=0` on re-mod B: output pads driven low (not floating). SX1302 expects a valid bitstream or quiescent state; driving low is safe.

**REMOD_CLK.** The 32 MHz `clk_32m` is also fanned out to SX1302 as `REMOD_CLK` — see [System Architecture](../System%20Diagram.md).
...
| Re-mod B disabled (NT=1) | `en=0` | `out_i` / `out_q` held low |
| Both re-mods simultaneously | Different inputs A and B | Independent outputs; no crosstalk |
